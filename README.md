...
### v1.0.2 (2026-02-06)
- **이모지 시스템 최종 고도화**: 80개 이상의 풍성한 이모지 데이터셋 구축 및 중복 제거.
- **스마트 추천 & 자동 선택**: 추천 테마 세트 전체(최대 5개) 자동 장착 기능 추가.
- **UI/UX 구역 정리**: 선택된 이모지가 입력창 내부에 장착되는 UI 및 입력창 하단 고정 픽커 구현.
- **편의성 보완**: 단계적 ESC 종료(픽커 -> 창), 영역 외 클릭 닫기, 안내 툴팁 추가.
